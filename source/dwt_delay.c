...
//---------------------------------------------//
//	Waits for delay
//	Delay time is set in microseconds
//---------------------------------------------//
void DWT_DelayUs(uint32_t time)
{
	time = DWT_StartDelayUs(time);
	while(DWT_DelayInProgress(time));
}

//---------------------------------------------//
//	Waits for delay
//	Delay time is set in milliseconds
//---------------------------------------------//
void DWT_DelayMs(uint32_t time)
{
	time = DWT_StartDelayUs(time * 1000);
	while(DWT_DelayInProgress(time));
}
